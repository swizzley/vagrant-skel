...
**THIS IS ONLY A FRAMEWORK, THIS WILL NOT WORK WITHOUT A PUPPET REPO, HIERA REPO, AND VALID CODE INCLUSION BASED ON $::role**
...
Vagrant files are written in ruby, and are used to configure and initiate the vm. They can be as simplistic as 2 lines for a single vm, or far more complex for multiple vms with different resources and provisioned using custom code. This is the basis for this repo. 

  * gui.rb (1 Gui Upstream Instance) 4G RAM
  * core.rb (1 Core Services Instance) 4G RAM *Only one tomcat instance will run due to memory*
  * bpol.rb ([ 1 rabbitmq, 1 allocator, 1 collector, 1 processor, 1 hbase ] instance) 2.5G RAM *In development because hbase isn't in puppet yet*
